// For editing set to 100, for rendering for print set to 300
$fn=300;
...
// This adds "raft" - it makes first layer solid and slightly bigger
// than the case itself. All openings should be opened with utility
// knife later. It adds uniform look to the case since there no
// visible lines around openings. It also adds support for cable
// relief.
FULL_RAFT = 0;
// For the front part enables "raft" but with opening that allow to
// use slicer tree support feature for the encoder. Has same effect
// as FULL_RAFT on the case back. It also adds support for cable
// relief.
PARTIAL_RAFT = 1;
// Turns on encoder support om the edge. May be useful in case slicer
// supports isn't used or if full raft is used.
ENCODER_SUPPORT = 0;
// If you have ST-Link debug port and want to use it set this 
// variable to 1
DEBUG_PORT = 0;
...
LAYER_W = 0.4; // Print layer width

// *******************************************************************
// ***   Render   ****************************************************
// *******************************************************************

// *** For printing - uncomment one at a time to render **************
Top();
//Bottom();
//CableReliefCircular(4); // Argument is diameter of used cable

// *** Different orientations for develoment *************************
//translate([75,0,0]) Bottom();
//translate([0,0,H+BOTTOM_H]) rotate([0,180,0]) Bottom();
//translate([-BW/2+(13-8)/2,BL-2,H-8]) cube([8,10,H]);

//#translate([-BW/2+12/2,L-W/2-R-1.2-2.4,11.5]) rotate([-90,0,0]) CableReliefCircular(4); // Argument is diameter of used cable
...
// *******************************************************************
// ***   Cable Relief Circular   *************************************
// *******************************************************************
module CableReliefCircular(D_CABLE)
{
  // Parameters
  BT = 1.2;
  T = 2.4;
  ...
  color("red") difference()
  {
    union()
    {
      translate([0,0,0]) cylinder(d=12, h=BT); // Bottom section
      translate([0,0,BT]) cylinder(d=8, h=T); // Middle section
      translate([0,0,BT+T]) cylinder(d=12, h=UT); // Top section
      translate([0,0,BT+T+UT]) cylinder(d1=10, d2 = D_CABLE+LAYER_W*2, h = 10); // Cone
      // Support
      if(FULL_RAFT || PARTIAL_RAFT)
      {
        difference()
        {
          cylinder(d=12, h=BT+T);
          translate([0,0,-1]) cylinder(d=12-LAYER_W*2, h=BT+T+UT);
        }
      }
    }
...
module CableRelief(D_CABLE)
{
  ...
  color("red") difference()
  {
    union()
    {
      translate([-6,-6+2.6,0]) cube([12, 12-2.6*2, BT]); // Bottom section
      translate([-4,-4,BT]) cube([8, 8, T]); // Middle section
      translate([-6,-6,BT+T]) cube([12, 12, UT]); // Top section
      translate([0,0,BT+T+UT]) cylinder(d1=10, d2 = D_CABLE+LAYER_W*2, h = 10); // Cone
      if(FULL_RAFT || PARTIAL_RAFT)
      {
        // Support middle cube
        difference()
        {
          translate([-4,-4,0]) cube([8, 8, BT]);
          translate([-4+LAYER_W,-4+LAYER_W,-1]) cube([8-LAYER_W*2, 8-LAYER_W*2, BT+2]);
        }
        // Support upper cube
        difference()
        {
...
module Top()
{
  ...
  if(PARTIAL_RAFT)
  {
    ...
    {
      CaseRaft(3, LAYER_H);
      translate([0,0,-1]) cylinder(d=W-3*2, h=H);
    }
  }
  // Encoder support
  if(ENCODER_SUPPORT)
...
//      translate([-BW/2+13,BL-2,BZPOS+BT]) cube([6,2+(R-T),H-(BZPOS+BT)]);
    ...
    hull()
    {
      translate([-BW/2+12/2,BL-2,H-8/2-0.5]) rotate([-90,0,0]) cylinder(d=8, h=H);
      translate([-BW/2+12/2-8/2,BL-2,H]) cube([8,8,1]);
    }
...
module Bottom()
{
  ...
  difference()
  {
    ...
    // Cable hole
    translate([+BW/2-12/2,BL+0.2-1.3,H+BOTTOM_H-(H-8/2-0.5)]) rotate([-90,0,0]) cylinder(d=12, h=1.3);
  }
  if(DEBUG_PORT)
  {
    // Debug port cutout(connector only)
  ...
  difference()
  {
    translate([+BW/2-12/2-8/2+0.1,BL+0.2,BOTTOM_H]) cube([8-0.2,T,4]);
    translate([+BW/2-12/2,BL+0.2-1.3,H+BOTTOM_H-(H-8/2-0.5)]) rotate([-90,0,0]) cylinder(d=8.2, h=10);
  }
  // Top lock
  difference()
  {
  ...
  difference()
  {
    ...
    difference()
    {
      union()
      {
        ...
        {
          LOCK_W = 6;
          translate([-BW/2,55.8-LOCK_W/2,0]) cube([3.5,LOCK_W,BOTTOM_H+H-BZPOS-BT]);
          translate([BW/2-3.5,55.8-LOCK_W/2,0]) cube([3.5,LOCK_W,BOTTOM_H+H-BZPOS-BT]);
        }
        else
        {
          LOCK_W = 8;
